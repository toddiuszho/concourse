...
var _ = Describe("A build using an image_resource", func() {
	...
	Describe("pipeline builds", func() {
		It("keeps images for the latest build", func() {
			...
			fly.Spawn("set-pipeline", "-n", "-c", "pipelines/build-image-gc-part-1.yml", "-p", "test")

			By("unpausing the pipeline")
			fly.Spawn("unpause-pipeline", "-p", "test")

			By("triggering a build that waits")
			watchSession := fly.Spawn("trigger-job", "-w", "-j", "test/some-job")
			...
			fly.Spawn("set-pipeline", "-n", "-c", "pipelines/build-image-gc-part-2.yml", "-p", "test")

			By("triggering a build that succeeds")
			fly.Spawn("trigger-job", "-w", "-j", "test/some-job")
