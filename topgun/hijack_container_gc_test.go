...
var _ = Describe("Hijacked containers", func() {
	...
	It("does not delete hijacked build containers from the database, and sets a 5 minute TTL on the container in garden", func() {
		...
		fly.Spawn("set-pipeline", "-n", "-c", "pipelines/task-waiting.yml", "-p", "hijacked-containers-test")

		By("triggering the build")
		fly.Spawn("unpause-pipeline", "-p", "hijacked-containers-test")
		buildSession := fly.Spawn("trigger-job", "-w", "-j", "hijacked-containers-test/simple-job")
	...
	It("does not delete hijacked resource containers from the database, and sets a 5 minute TTL on the container in garden", func() {
		By("setting the pipeline that has a build")
		fly.Spawn("set-pipeline", "-n", "-c", "pipelines/get-task.yml", "-p", "hijacked-resource-test")
		fly.Spawn("unpause-pipeline", "-p", "hijacked-resource-test")

		By("checking resource")
		fly.Spawn("check-resource", "-r", "hijacked-resource-test/tick-tock")
		...
		fly.Spawn("set-pipeline", "-n", "-c", "pipelines/task-waiting.yml", "-p", "hijacked-resource-test")
