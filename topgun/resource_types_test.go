...
var _ = Describe("A pipeline-provided resource type", func() {
	...
	AfterEach(func() {
		...
		// fly.Spawn("prune-worker", "-w", workers[0])
	...
	It("does not result in redundant containers when running resource actions", func() {
		...
		fly.Spawn("set-pipeline", "-n", "-c", "pipelines/custom-types.yml", "-p", "pipe")
		fly.Spawn("unpause-pipeline", "-p", "pipe")

		By("triggering the build")
		buildSession := fly.Spawn("trigger-job", "-w", "-j", "pipe/get-10m")
		<-buildSession.Exited
		...
		buildSession = spawnfly.Spawn("trigger-job", "-w", "-j", "pipe/get-10m")
		<-buildSession.Exited
		Expect(buildSession.ExitCode()).To(Equal(1))
...
var _ = Describe("tagged resource types", func() {
	BeforeEach(func() {
		...
		fly.Spawn("set-pipeline", "-n", "-c", "pipelines/tagged-custom-types.yml", "-p", "pipe")
		fly.Spawn("unpause-pipeline", "-p", "pipe")
	})

	It("is able to be used with tagged workers", func() {
		By("running a check which uses the tagged custom resource")
		Eventually(fly.Spawn("check-resource", "-r", "pipe/10m")).Should(gexec.Exit(0))
