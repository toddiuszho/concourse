...
var _ = Describe("Resource checking", func() {
	...
	Context("with tags on the resource", func() {
		BeforeEach(func() {
			...
			fly.Spawn("set-pipeline", "-n", "-c", "pipelines/tagged-resource.yml", "-p", "tagged-resource")

			By("unpausing the pipeline pipeline")
			fly.Spawn("unpause-pipeline", "-p", "tagged-resource")
		})

		It("places the checking container on the tagged worker", func() {
			By("running the check")
			fly.Spawn("check-resource", "-r", "tagged-resource/some-resource")
