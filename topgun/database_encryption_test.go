...
var _ = Describe("Database secrets encryption", func() {
	configurePipelineAndTeamAndTriggerJob := func() {
		...
		fly.Spawn("set-pipeline", "-n", "-c", "pipelines/secrets.yml", "-p", "pipeline-secrets-test")
		fly.Spawn("unpause-pipeline", "-p", "pipeline-secrets-test")
		...
		buildSession := fly.Spawn("trigger-job", "-w", "-j", "pipeline-secrets-test/simple-job")
		<-buildSession.Exited
	...
	getPipeline := func() *gexec.Session {
		session := fly.Spawn("get-pipeline", "-p", "pipeline-secrets-test")
		<-session.Exited
		Expect(session.ExitCode()).To(Equal(0))
		return session
	}
