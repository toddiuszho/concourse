...
var _ = Describe("Vault", func() {
	...
	getPipeline := func() *gexec.Session {
		session := fly.Spawn("get-pipeline", "-p", "pipeline-vault-test")
		<-session.Exited
		Expect(session.ExitCode()).To(Equal(0))
		return session
	}
		...
		Context("with token auth", func() {
			...
			Context("with a pipeline build", func() {
				BeforeEach(func() {
					...
					fly.Spawn("set-pipeline", "-n", "-c", "pipelines/credential-management.yml", "-p", "pipeline-vault-test")
					...
					fly.Spawn("unpause-pipeline", "-p", "pipeline-vault-test")
				})

				It("parameterizes via Vault and leaves the pipeline uninterpolated", func() {
					By("triggering job")
					watch := fly.Spawn("trigger-job", "-w", "-j", "pipeline-vault-test/job-with-custom-input")
					Wait(watch)
				...
				Context("when the job's inputs are used for a one-off build", func() {
					It("parameterizes the values using the job's pipeline scope", func() {
						...
						watch := fly.Spawn("trigger-job", "-w", "-j", "pipeline-vault-test/job-with-input")
						Wait(watch)
						Expect(watch).To(gbytes.Say("GET SECRET: GET-Hello/GET-World"))
						Expect(watch).To(gbytes.Say("PUT SECRET: PUT-Hello/PUT-World"))
						Expect(watch).To(gbytes.Say("GET SECRET: PUT-GET-Hello/PUT-GET-World"))
						...
						By("executing a task that parameterizes image_resource")
						watch = fly.Spawn("execute", "-c", "tasks/credential-management-with-job-inputs.yml", "-j", "pipeline-vault-test/job-with-input")
						Wait(watch)
